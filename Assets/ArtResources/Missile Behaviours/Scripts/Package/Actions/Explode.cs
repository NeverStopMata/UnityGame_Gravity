    ...
    public class Explode : ActionBase
    {
        ...
        internal override void OnTrigger(object sender, EventArgs e)
        {
            if (!wasTriggered && this.enabled)
            {
                ...
                BattleControlCenter bcc = transform.parent.GetComponent<BattleControlCenter>();
                foreach (GameObject go in bcc.movalbeGOs)
                {
                    Vector3 forceDrct = Vector3.Normalize(go.transform.position - transform.position);
                    go.GetComponent<Rigidbody>().AddForce(forceDrct * 20.0f, ForceMode.Impulse);
                }
